...
2. Use [combinational process](https://github.com/tomas-fryza/vhdl-examples/wiki/Processes) and complete an architecture of the decoder.
   ...
   In the process sensitivity list are declared all the signal which the process is sensitive to. In the following example, the process is evaluated any time a transaction is scheduled on the signal `bin` or `clear`. Inside a process, `case`-`when` [assignments](https://github.com/tomas-fryza/vhdl-examples/wiki/Signal-assignments) can be used.
...
4. Create a new [constraints XDC](https://raw.githubusercontent.com/Digilent/digilent-xdc/master/Nexys-A7-50T-Master.xdc) file `nexys` and uncomment used pins according to the `top_level` entity.
